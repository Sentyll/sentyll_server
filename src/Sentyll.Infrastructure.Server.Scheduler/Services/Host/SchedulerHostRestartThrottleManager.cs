...
internal sealed class SchedulerHostRestartThrottleManager
{
    ...
    public SchedulerHostRestartThrottleManager(Action onRestartTriggered)
    {
    ...
    public void RequestRestart()
    {
        ...
        lock (_lock)
        {
            ...
            else
            {
                ...
                {
                    _extraDelay = TimeSpan.FromMilliseconds(Math.Min((_extraDelay.TotalMilliseconds * 2) + 50, _maxExtraDelay.TotalMilliseconds));
                }
                else
                {
                    _extraDelay = TimeSpan.Zero;
