...
internal sealed class ServerEventDispatcher(
    IEnumerable<IServerEventProcessor> eventProcessors,
    ILogger<ServerEventDispatcher> logger
) : IEventDispatcher
{
     ...
     public async Task<Result> HandleAsync(HealthCheckJobExecutionEvent jobExecutionEvent,
        ...
        => await Result
            ...
            {
                logger.LogWarning("No server event processors are available to consume health check execution queue events.");
                return Result.Failure("No server event Processors");
            })
